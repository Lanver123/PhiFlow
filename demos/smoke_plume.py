...
DOMAIN = dict(x=sim_precision, y=sim_precision, bounds=Box[0:100, 0:100])
velocity = StaggeredGrid((0, 0), extrapolation.BOUNDARY, **DOMAIN)  # or use CenteredGrid
smoke = CenteredGrid(0, extrapolation.BOUNDARY, x=sim_precision, y=sim_precision, bounds=DOMAIN['bounds'])
...
def step(smoke, velocity, pressure):
    ...
    with math.SolveTape() as tape:
        velocity, pressure = fluid.make_incompressible(velocity, (), Solve('CG', err_thresh, err_thresh, max_iterations=4000, x0=pressure))
